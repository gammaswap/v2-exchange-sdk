...
const MNEMONIC =
  process.env.TEST_MNEMONIC || "test test test test test test test test test test junk";
...
async function main() {
  ...
  const res = await client.placeAgentOrder({
    assetId: ASSET_ID,
    epoch: EPOCH,
  ...
  console.log("signedAgentOrderMessage:", res.request);
