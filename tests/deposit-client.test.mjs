...
test("settlement token amount parser converts decimal strings to six-decimal base units", () => {
  ...
  assert.equal(parseSettlementTokenAmount("100.000001"), 100_000_001n);
  assert.equal(parseSettlementTokenAmount("0.000001"), 1n);
...
test("settlement token amount parser rejects unsafe amount inputs", () => {
  ...
  assertProtocolError(() => parseSettlementTokenAmount("1.0000001"), "invalid_decimal_string");
...
test("DepositClient resolves the default localhost DepositLedger address", () => {
  ...
  assert.equal(client.parseAmount("100"), 100_000_000n);
});
