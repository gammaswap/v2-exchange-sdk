import type { DecodedAssetId } from "./types.js";
...
export function decodeAssetId(assetId: bigint): DecodedAssetId {
  const id = String((assetId >> BigInt(ID_OFFSET)) & ID_MASK);
...
export function parseExpirationTf(expirationTf: string): number {
  ...
  const match = trimmed.match(/^(\d+)([sSmMhHdDwWyY])$/);
  if (!match) {
    throw new Error(
      `Invalid expiration timeframe format: ${expirationTf}. Expected format: <number><unit> (e.g., "15m", "1h", "1d", "1w", "1M", "1y")`,
    );
  }
  ...
  switch (unit) {
    ...
    default:
      throw new Error(
        `Invalid expiration timeframe unit: ${unit}. Must be one of: s, m, h, d, w, M, y`,
      );
  }
}
