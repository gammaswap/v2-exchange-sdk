...
export const DEPOSIT_SWEEP_ORDER_TYPEHASH = keccak256(Buffer.from(
    "DepositSweepOrder(uint8 typ,address owner,uint64 amount,uint128 index,address ledger)"
));

export const DEPOSIT_ORDER_TYPEHASH = keccak256(Buffer.from(
...
export const WITHDRAWAL_ORDER_TYPEHASH = keccak256(Buffer.from(
...
export const FILL_ORDER_TYPEHASH = keccak256(Buffer.from(
...
export const CANCEL_ORDER_TYPEHASH = keccak256(Buffer.from(
...
export const RESOLUTION_ORDER_TYPEHASH = keccak256(Buffer.from(
...
export const CLAIM_ORDER_TYPEHASH = keccak256(Buffer.from(
...
export const REVOKE_AGENT_ORDER_TYPEHASH = keccak256(Buffer.from(
    "RevokeAgentOrder(uint8 typ,uint64 nonce,address signer,uint8 signatureType,address sender)"
));

export const AGENT_APPROVAL_TYPEHASH = keccak256(Buffer.from(
    "AgentApproval(address master,address agent,uint32 nonce)"
));