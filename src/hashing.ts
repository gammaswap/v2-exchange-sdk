...
export function getExchangeDomain(chainId: string | number | bigint = CHAIN_ID, verifyingContract: string = String(EXCHANGE_DOMAIN.verifyingContract)): TypedDataDomain {
    return {
        name: "GammaSwap Exchange",
        version: "2",
...
export function hashTypedDataStruct(structHash: string, domain: TypedDataDomain = EXCHANGE_DOMAIN): string {
    return keccak256(
        solidityPacked(
