...
async function main() {
    ...
    const replacementSize = args[2] && Number(args[2]) > 0 ?
        BigInt(Math.floor(Number(args[2]))) :
        100_000_000n;
    const replacementPrice = args[3] && Number(args[3]) > 0 ?
        BigInt(Math.floor(Number(args[3]))) :
        930000n;
    ...
    console.log("isReplacementRecovered:", validateSignatureJS(replacementOrderHash, replacementSignature, wallet.address));
    ...
    console.log("isCancelReplaceRecovered:", validateSignatureJS(orderHash, signature, wallet.address));
